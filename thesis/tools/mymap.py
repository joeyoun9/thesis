...
from math import sqrt, radians, pow, sin, cos, tan
import numpy as np
import logging as l
...
def ll2utm(lat, lon, zone=False):
	...
	x = E0 + k0 * a * _v * (_A + (1 - _T + _C) * (pow(_A, 3) / 6) \
		+ (5 - 18 * _T + _T * _T) * (pow(_A, 5) / 120))
	y = N0 + k0 * a * (_s + _v * tan(lat) * (_A * _A / 2 \
		+ (5 - _T + 9 * _C + 4 * _C * _C) * pow(_A, 4) / 24 \
		+ (61 - 58 * _T + _T * _T) * pow(_A, 6) / 720))
...
def find_zone(lon):
	...
	for z in range(1, 61):
		if v <= z * (360 / 60):
			zone = z
			break
	l.debug("Zone identified: " + str(zone))
	return zone
